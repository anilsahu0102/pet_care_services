...
@section('content')
	...
	<div class="row">
		<div class="col-md-3">
			<a type="button" class="btn btn-primary" href="{{{ URL::to('pet_type/add') }}}" title="Add Pet Type">Add Pet Type</a>
		</div>
	</div>
	<br/>
	<div class="table-responsive">
		<table class="table table-bordered table-hover">
			...
			<tbody>
				@if(count($a_pet_types) > 0)
					@foreach($a_pet_types as $row)
						<tr>
							<td>{{ $row->pet_name }}</td>
							...
							<div class="modal-dialog modal-sm">
								<div class="modal-content">
									...
									<div class="modal-body">
										<p>Are you sure to delete the pet type?</p>
									</div>
									<div class="modal-footer">
										<button type="button" class="btn btn-default" data-dismiss="modal">No</button>
										<a  class="btn btn-primary" href="{{ URL::to('pet_type/delete/' . $row->id) }}" >Yes</a>
									</div>
								</div>
							</div>
						</div>
